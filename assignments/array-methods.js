...
let largeShirts = [];
const largeShirt  = runners.filter(function(runners) {
...
/* Now that you have used .forEach(), .map(), .filter(), and .reduce().  I want
 you to think of potential problems you could solve given the data set and the
 5k fun run theme.  Try to create and then solve 3 unique problems using one or
 many of the array methods listed above. */

// Problem 1
//Create a new array displaying their Full Name, Company, and Email for contact information
let contactInfo = [];
runners.forEach(function(runner) {
 contactInfo.push(`${runner.first_name} ${runner.last_name} : ${runner.company_name} : ${runner.email}`);
});
console.log(contactInfo);

// Problem 2
//Let's see who our biggest benefactor is, sort the array in order from biggest to lowest donation


const benefactor = runners.map(function(runners){
  return runners.donation;
});
const biggestBenefactor = benefactor.sort(function(a, b){return b-a});


// Problem 3
//Retrieve each ID of the runner, and put it in a string saying " I am number ${}"
let runnerID = [];
runners.forEach(function(runner) {
 runnerID.push(`I am number ${runner.id}`);
